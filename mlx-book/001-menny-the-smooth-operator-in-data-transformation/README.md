...
This demonstrates the remarkable power of GPT-4. It's an impressive display of few-shot learning in action. However, her context window so tiny to my likings and custom instructions are limited and consume her context window at every interaction. So there's clear limits to this approach. As I already mentioned on a number of occasions, RAG is not a solution to the inherent limits in the transformer's architecture. RAG'd info is not learned parameters, it's just a given context. Nothing more. I seldom use GPTs for web searches. When I need to do searches, I resort to googling or Perplexity.ai for good reasons. RAGs don't cut it.
...
[README.md](..%2F..%2Fbook%2FREADME.md)
